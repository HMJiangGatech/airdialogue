...
from airdialogue.evaluator.evaluator_main import tokenize_kb, action_obj_to_str
from airdialogue.evaluator.selfplay_utils import compute_reward

from airdialogue.simulator_ope import interaction
import random
import numpy as np
...
def add_arguments(parser):
  ...
  parser.add_argument("--seed", type=int, default=0,
                      help="Seed for random generater")

  # parameters for generate OPE data
  parser.add_argument("--n_policies", type=int, default=6,
                      help="Number of different policies for OPE (default: 6)")
...
def main(FLAGS):
  ...
  for tgt_policy,ref_policy in itertools.permutations(range(FLAGS.n_policies),2):
    ...
    ref_policy = "L"+str(ref_policy)
    if ref_policy not in all_rewards:
        all_rewards[ref_policy] = []
    if FLAGS.verbose:
      print("Target Policy ID: "+tgt_policy+" Accuracy: "+str(tgt_policy_accu))
      print("Reference Policy ID: "+ref_policy+" Accuracy: "+str(ref_policy_accu))
      print(stats)
    gfile.MakeDirs(FLAGS.output_dir+'/tgt_'+tgt_policy)
    output_data = FLAGS.output_dir+'/tgt_{}/data_ref_{}.json'.format(tgt_policy,ref_policy)
    output_kb = FLAGS.output_dir+'/tgt_{}/kb_ref_{}.json'.format(tgt_policy,ref_policy)
    with gfile.Open(output_data, "w") as f_data, \
            gfile.Open(output_kb, "w") as f_kb:
      for i in range(len(ct)):
        if FLAGS.verbose and i % 5000 == 0:
          print((i, "/", len(ct)))
        cus, kb, expected_action = ct[i]
        # action has been standarlized in inter
        utterance, action, _ = inter.generate_dialogue(cus, kb, ref_policy_accu,tgt_policy_accu)
        ...
        reward = get_reward(standarlized_action, standarlized_expected_action, dumped_kb)
        all_rewards[ref_policy].append(reward)
        f_data.write(
  ...
  if FLAGS.verbose:
    for k,v in all_rewards.items():
      v=np.array(v)
      print("{} Policy Avg Reward: {}, Std: {}".format(k,v.mean(),v.std()))


def get_reward(action, expected_action, kb):
  ss = compute_reward(
      action_obj_to_str(action), action_obj_to_str(expected_action),
      tokenize_kb(kb)
  )
  return ss[0]
